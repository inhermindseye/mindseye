...
categories: 
  - MEAD
  - apprentices
slug: mead-pauper-apprentices-philadelphia-1751-99
ihmefilename: "201804_mead_1"
execute: 
  warning: false
  message: false
  echo: false
...
# original tsv
# mead_apprentices_philadelphia_201804.tsv" 
...
# sourced from another MEAD dataset https://repository.upenn.edu/mead/5/ - it was missing from the apprentices dataset but I think this is correct data
# "mead_occupational_codes_201804.csv"

# prepared data 

# add unique id to each row 
# year apprenticed is two digits in the data - convert to full year
# add decade
# master_fname - for matching gender
...
mead_app <-
  read_csv(here::here("site_data/mead/mead_apprentices_processed.csv"))

pc_girls_7 <- 
  mead_app |> 
  filter(YearsApprenticed==7) |> 
  add_count(YearsApprenticed) |>
  count(YearsApprenticed, GenderApprentice, n) |> 
  mutate(pc = round(nn/n*100,2) ) |> 
  filter(GenderApprentice=="female") |> 
  select(pc)
```
...
kable(mead_app |> 
        slice(100:109) |> 
        select(year_apprenticed, decade_apprenticed, YearsApprenticed, GenderApprentice, GenderMaster, Occupation) |> 
        arrange(year_apprenticed) )
```
...
```{r mead-bar-gender-masters-1, out.width="60%"}

mead_app |> 
  filter(grepl("male",GenderMaster) ) |>  
  count(GenderMaster) |>
ggplot(aes(x="", y=n, fill=GenderMaster)) +
...
```{r mead-bar-gender-apprentices-1, out.width="60%"}
mead_app |> 
  count(GenderApprentice) |>
  ggplot(aes(fill=GenderApprentice, y=n, x="")) +
  geom_bar(stat = "identity", position="fill") +
   scale_y_continuous(labels = percent_format())  +
  scale_fill_tableau() +
...
```{r mead-bar-gender-master-apprentices-1}
...
```{r mead-histogram-years-apprenticed}
...
```{r mead-bar-decade-years-apprenticed}
mead_app |> 
  count(decade_apprenticed, years_apprenticed) |>
  ggplot(aes(x=decade_apprenticed, y=n, fill=years_apprenticed) ) +
  geom_bar(stat = "identity", position = "fill") +
  scale_y_continuous(labels = percent_format()) +
  scale_fill_tableau() +
...
```{r mead-bar-gender-years-apprenticed}
mead_app_summary_gender_yearsapp <-
  ...
  ggplot(aes( x=years_apprenticed, y=div_years, fill=GenderApprentice)  ) +
...
```{r mead-bar-fill-gender-apprenticed-years-apprenticed}
...
```{r mead-bar-occupations-apprentices}
mead_app |> count(Occupation) |>
...
```{r mead-bar-occupation-gender-apprentices}
mead_app |> 
  add_count(Occupation) |> 
  count(Occupation, GenderApprentice, n, name="nn") |> 
  filter(n>3) |>
  ggplot(aes(x=reorder(Occupation, n), y=nn, fill=GenderApprentice)) +
  geom_bar(stat="identity") +
  coord_flip() +
  scale_fill_tableau() +
...
<!-- The data and R code for the post are available at the site's [Github repository](https://github.com/sharonhoward/ihme_data). -->
